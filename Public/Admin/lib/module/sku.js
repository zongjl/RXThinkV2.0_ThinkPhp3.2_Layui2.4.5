...
;layui.define(['jquery','form'],function(e){
    "use strict";var $ = layui.jquery,form=layui.form,o ={
        ...
        sth:function () {
            var h = '<tr>'; for(var i=0;i<this.c.tableTitle.length;i++){ h += '<th style="text-align:center;">' +this.c.tableTitle[i]+ '</th>';} h += '<th style="text-align:center;">价格</th><th style="text-align:center;">库存</th><th style="text-align:center;">货号</th><th style="text-align:center;">默认SKU</th><th style="text-align:center;">SKU图集</th></tr>'; return h;
        },
        stb:function () {
            var t =this, h = '', na = [];
            for(var i=0;i<t.c.data.length;i++){
                ...
                for(var s=0;s<t.c.data[i].ids.length;s++){
                    ...
                    if(!na[k] || na[k]<1){
                        ...
                        if(na[k]>0){
                            h += '<td rowspan="'+na[k]+'" style="text-align:center;">';
                ...
                h += '<td><input name="sex" data-index="'+i+'" data-type="is_default" value="'+t.c.data[i].sku+'" title="默认SKU" checked="" type="radio"></td>';
                h += '<td style="text-align:center;"><button type="button" class="layui-btn" onclick="uploadImgs('+t.c.data[i].id+');"><i class="layui-icon">&#xe67c;</i>上传图片</button></td>';
                h += '</tr>';
            }
            return h;
        },
        ...
        ced:function(ids){
            ...
            for(var i=0;i<this.c.data.length;i++){
                ...
                if(is){
                	arr['id'] = parseFloat(this.c.data[i].id);
        ...
        l:function () {
            ...
            $('#'+this.c.sv).append('<div class="layui-form-item" pane><label class="layui-form-label">商品SKU：</label><div class="layui-input-block" id="'+this.c.tv+'">'+this.st()+'</div></div></div>');
 ...
 * @returns
 */
function uploadImgs(skuId){
	layui.use(['func'],function(){
		var func = layui.func;
		var url = cUrl + "/skuImgs?sku_id="+skuId;
		func.showWin("上传SKU图集",url,850,450);
	})
}
